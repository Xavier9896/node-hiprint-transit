...
import { b as getDefaultExportFromCjs, c as commonjsGlobal, a as requireSupportsColor, I as I18n } from './index_chunk.js';
...
var OperatorSubscriber = (function (_super) {
    __extends$j(OperatorSubscriber, _super);
...
var ConnectableObservable = (function (_super) {
    __extends$i(ConnectableObservable, _super);
...
var AnonymousSubject = (function (_super) {
    __extends$h(AnonymousSubject, _super);
...
var BehaviorSubject = (function (_super) {
    __extends$g(BehaviorSubject, _super);
...
var ReplaySubject = (function (_super) {
    __extends$f(ReplaySubject, _super);
...
var AsyncSubject = (function (_super) {
    __extends$e(AsyncSubject, _super);
...
var AsyncAction = (function (_super) {
    __extends$c(AsyncAction, _super);
...
var AnimationFrameAction = (function (_super) {
    __extends$6(AnimationFrameAction, _super);
...
var AnimationFrameScheduler = (function (_super) {
    __extends$5(AnimationFrameScheduler, _super);
...
function callOrApply(fn, args) {
    return isArray$4(args) ? fn.apply(void 0, __spreadArray$f([], __read$g(args))) : fn(args);
}
    ...
    return function () {
        ...
        return new Observable_1$e.Observable(function (subscriber) {
            ...
            if (uninitialized) {
                ...
                callbackFunc.apply(_this, __spreadArray$e(__spreadArray$e([], __read$f(args)), [
                    function () {
                        var results = [];
                        for (var _i = 0; _i < arguments.length; _i++) {
...
function concat$2() {
    ...
    return concatAll_1$1.concatAll()(from_1$5.from(args, args_1$a.popScheduler(args)));
}
...
function fromEvent(target, eventName, options, resultSelector) {
    ...
    var _a = __read$e(isEventTarget(target)
...
function merge$2() {
    ...
    return !sources.length
        ...
        : sources.length === 1
            ...
            :
                mergeAll_1$1.mergeAll(concurrent)(from_1$4.from(sources, scheduler));
}
...
function concat() {
    ...
    return lift_1$P.operate(function (source, subscriber) {
        concatAll_1.concatAll()(from_1$2.from(__spreadArray$a([source], __read$a(args)), scheduler)).subscribe(subscriber);
    });
}
...
function first(predicate, defaultValue) {
    var hasDefaultValue = arguments.length >= 2;
    return function (source) {
        return source.pipe(predicate ? filter_1$2.filter(function (v, i) { return predicate(v, i, source); }) : identity_1$7.identity, take_1.take(1), hasDefaultValue ? defaultIfEmpty_1$1.defaultIfEmpty(defaultValue) : throwIfEmpty_1$1.throwIfEmpty(function () { return new EmptyError_1$2.EmptyError(); }));
    };
}
first$1.first = first;
...
function last(predicate, defaultValue) {
    var hasDefaultValue = arguments.length >= 2;
    return function (source) {
        return source.pipe(predicate ? filter_1$1.filter(function (v, i) { return predicate(v, i, source); }) : identity_1$6.identity, takeLast_1.takeLast(1), hasDefaultValue ? defaultIfEmpty_1.defaultIfEmpty(defaultValue) : throwIfEmpty_1.throwIfEmpty(function () { return new EmptyError_1$1.EmptyError(); }));
    };
}
last$1.last = last;
...
function merge() {
    ...
    return lift_1$t.operate(function (source, subscriber) {
        mergeAll_1.mergeAll(concurrent)(from_1$1.from(__spreadArray$7([source], __read$7(args)), scheduler)).subscribe(subscriber);
    });
}
merge$1.merge = merge;
...
function onErrorResumeNextWith() {
    ...
    return function (source) { return onErrorResumeNext_1.onErrorResumeNext.apply(void 0, __spreadArray$5([source], __read$5(nextSources))); };
}
...
function observe(rl) {
  const keypress = cjs.fromEvent(rl.input, 'keypress', normalizeKeypressEvents)
    ...
    .pipe(cjs.filter(({ key }) => key.name !== 'enter' && key.name !== 'return'));
...
class Paginator {
  ...
  paginate(output, active, pageSize) {
    ...
    return (
      visibleLines.join('\n') +
      '\n' +
      colors$1.dim('(Move up and down to reveal more choices)')
    );
  }
...
class Separator {
  constructor(line) {
    this.type = 'separator';
    this.line = colors$1.dim(line || Array.from({ length: 15 }).join(figures.line));
  }
...
class Choices {
   ...
   */
  getChoice(selector) {
    assert$1(typeof selector === 'number');
...
const chalk$3 = Chalk$1(); // eslint-disable-line new-cap
chalk$3.supportsColor = stdoutColor$1;
chalk$3.stderr = Chalk$1({level: stderrColor$1 ? stderrColor$1.level : 0}); // eslint-disable-line new-cap
chalk$3.stderr.supportsColor = stderrColor$1;

var source$1 = chalk$3;
...
const { Buffer: Buffer$2 } = require$$0$6;
...
class ScreenManager {
  ...
  breakLines(lines, width = this.normalizedCliWidth()) {
    ...
    return lines.map((line) =>
      wrapAnsi$1(line, width, { trim: false, hard: true }).split('\n'),
    );
  }
...
class Prompt {
  ...
  handleSubmitEvents(submit) {
    ...
    const success = validation.pipe(
      cjs.filter((state) => state.isValid === true),
      cjs.take(1),
    );
  ...
  getQuestion() {
    let message =
      ...
      colors$1.bold(this.opt.message) +
      this.opt.suffix +
      colors$1.reset(' ');
    ...
    if (
      ...
      message +=
        this.opt.type === 'password'
          ? colors$1.italic(colors$1.dim('[hidden] '))
          : colors$1.dim('(' + this.opt.default + ') ');
    }
...
class ListPrompt extends Prompt {
  ...
  _run(cb) {
    ...
    events.normalizedDownKey
      .pipe(cjs.takeUntil(events.line))
      .forEach(this.onDownKey.bind(this));
    events.numberKey.pipe(cjs.takeUntil(events.line)).forEach(this.onNumberKey.bind(this));
    events.line
      .pipe(
        cjs.take(1),
  ...
  render() {
    ...
    if (this.firstRender) {
      message += colors$1.dim('(Use arrow keys)');
    }

    // Render choices or answer depending on the state
    if (this.status === 'answered') {
      message += colors$1.cyan(this.opt.choices.getChoice(this.selected).short);
    } else {
...
class InputPrompt extends Prompt {
  ...
  _run(cb) {
    ...
    const events = observe(this.rl);
    const submit = events.line.pipe(cjs.map(this.filterInput.bind(this)));
    ...
    events.keypress
      .pipe(cjs.takeUntil(validation.success))
  ...
  render(error) {
    ...
    if (transformer) {
      ...
      message += isFinal ? colors$1.cyan(appendContent) : appendContent;
    }

    if (error) {
      bottomContent = colors$1.red('>> ') + error;
    }
...
class ConfirmPrompt extends Prompt {
  ...
  _run(cb) {
    ...
    // Once user confirm (enter key)
    const events = observe(this.rl);
    events.keypress.pipe(cjs.takeUntil(events.line)).forEach(this.onKeypress.bind(this));

    events.line.pipe(cjs.take(1)).forEach(this.onEnd.bind(this));
  ...
  render(answer) {
    ...
    if (typeof answer === 'boolean') {
      message += colors$1.cyan(answer ? 'Yes' : 'No');
    } else if (answer) {
      message += answer;
    } else {
...
class RawListPrompt extends Prompt {
  ...
  _run(cb) {
    ...
    // Once user confirm (enter key)
    const events = observe(this.rl);
    const submit = events.line.pipe(cjs.map(this.getCurrentValue.bind(this)));
...
class ExpandPrompt extends Prompt {
  ...
  _run(cb) {
    ...
    const validation = this.handleSubmitEvents(
      events.line.pipe(cjs.map(this.getCurrentValue.bind(this))),
    );
  ...
  render(error, hint) {
    ...
    if (this.status === 'answered') {
      message += colors$1.cyan(this.answer);
    ...
    if (hint) {
      bottomContent = colors$1.cyan('>> ') + hint;
    }
  ...
  getChoices() {
    ...
    this.opt.choices.forEach((choice) => {
      ...
      if (this.selectedKey === choice.key) {
        choiceStr = colors$1.cyan(choiceStr);
      }
...
class CheckboxPrompt extends Prompt {
  ...
  _run(cb) {
    ...
    events.aKey.pipe(cjs.takeUntil(validation.success)).forEach(this.onAllKey.bind(this));
    events.iKey.pipe(cjs.takeUntil(validation.success)).forEach(this.onInverseKey.bind(this));
  ...
  render(error) {
    ...
    if (!this.dontShowHints) {
      message +=
        '(Press ' +
        colors$1.cyan(colors$1.bold('<space>')) +
        ' to select, ' +
        colors$1.cyan(colors$1.bold('<a>')) +
        ' to toggle all, ' +
        colors$1.cyan(colors$1.bold('<i>')) +
        ' to invert selection, and ' +
        colors$1.cyan(colors$1.bold('<enter>')) +
        ' to proceed)';
    }

    // Render choices or answer depending on the state
    if (this.status === 'answered') {
      message += colors$1.cyan(this.selection.join(', '));
    } else {
    ...
    if (choice.disabled) {
      ...
      output +=
        i - separatorOffset === pointer
          ? colors$1.cyan(figures.pointer + line)
          : ' ' + line;
    }
...
function getCheckbox(checked) {
  return checked ? colors$1.green(figures.radioOn) : figures.radioOff;
}
  ...
  getMaskedValue(value) {
    if (this.status === 'answered') {
      return this.opt.mask
        ? colors$1.cyan(mask(value, this.opt.mask))
        : colors$1.italic(colors$1.dim('[hidden]'));
    }
    return this.opt.mask
      ? mask(value, this.opt.mask)
      : colors$1.italic(colors$1.dim('[input is hidden] '));
  }
...
class EditorPrompt extends Prompt {
  ...
  render(error) {
    ...
    message +=
      this.status === 'answered'
        ? colors$1.dim('Received')
        : colors$1.dim('Press <enter> to launch your preferred editor.');
...
class PromptUI extends UI {
  ...
  run(questions, answers) {
    ...
    if (Array.isArray(questions)) {
      obs = cjs.from(questions);
    } else if (cjs.isObservable(questions)) {
      obs = questions;
    } else if (
    ...
    this.process = obs.pipe(
      cjs.concatMap(this.processQuestion.bind(this)),
      cjs.publish(), // Creates a hot Observable. It prevents duplicating prompts.
    );
  ...
  processQuestion(question) {
    question = { ...question };
    return cjs.defer(() => {
      const obs = cjs.of(question);

      return obs.pipe(
        cjs.concatMap(this.setDefaultType.bind(this)),
        cjs.concatMap(this.filterIfRunnable.bind(this)),
        cjs.concatMap(() =>
        ...
        cjs.concatMap(this.fetchAnswer.bind(this)),
      );
    });
  }
  ...
  fetchAnswer(question) {
    ...
    return cjs.defer(() =>
      cjs.from(this.activePrompt.run().then((answer) => ({ name: question.name, answer }))),
    );
  }
  ...
  setDefaultType(question) {
    ...
    return cjs.defer(() => cjs.of(question));
  }
  ...
  filterIfRunnable(question) {
    ...
    return cjs.defer(() =>
      cjs.from(
        runAsync$1(question.when)(answers).then((shouldRun) => {
          if (shouldRun) {
            return question;
          }
        }),
      ).pipe(cjs.filter((val) => val != null)),
    );
  }
}
